...
// グループ単位の収入管理API
// グループの収入一覧取得
router.post('/:groupId/incomes/list', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { groupId } = req.params;
    ...
    if (!membership) {
      res.status(403).json({ error: 'このグループの収入にアクセスする権限がありません' });
      return;
    }
    ...
    console.error('グループ収入一覧取得エラー:', error);
...
router.post('/:groupId/expenses/list', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (!membership) {
      res.status(403).json({ error: 'このグループの支出にアクセスする権限がありません' });
      return;
    }
